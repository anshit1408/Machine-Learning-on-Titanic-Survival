...
import numpy as np
import pandas as pd
pd.set_option('display.width', 1000) #1000 pixel data in just single row.
...
#Age has less count. This means age has missing data. For this we will use feature enginering.
...
print train.dtypes #String values are called object(datatype) here.
#OUTPUT:-
#PassengerId      int64
#Survived         int64
#Pclass           int64
...
#           PassengerId  Survived  Pclass                       Name   Sex     Age   SibSp   Parch    Ticket    Fare        Cabin Embarked
#   count        891.00    891.00  891.00                        891   891  714.00  891.00  891.00       891  891.00          204      889
#   unique          NaN       NaN     NaN                        891     2     NaN     NaN     NaN       681     NaN          147        2
#   top             NaN       NaN     NaN  Graham, Mr. George Edward  male     NaN     NaN     NaN  CA. 2343     NaN  C23 C25 C27        S
#   freq            NaN       NaN     NaN                          1   577     NaN     NaN     NaN         7     NaN            4      644
#   mean         446.00      0.38    2.31                        NaN   NaN   29.70    0.52    0.38       NaN   32.20          NaN      NaN
#   std          257.35      0.49    0.84                        NaN   NaN   14.53    1.10    0.81       NaN   49.69          NaN      NaN
...
#   75%          668.50      1.00    3.00                        NaN   NaN   38.00    1.00    0.00       NaN   31.00          NaN      NaN
#   max          891.00      1.00    3.00                        NaN   NaN   80.00    8.00    6.00       NaN  512.33          NaN      NaN
#Uniqueness of ticket is less than 891. This means some of the passengers have same PNR means they are family members.
#Top in sex means there are more no. of males. freq means the no. of males. Simlilarly in all other columns.
...
plt.show()#Seaborn gives many details on the graph on its own. Like x and y axis title it takes from column name and many more things it does itself.
#print percentages of females vs. males that survive
#print "Percentage of females who survived:", train["Survived"][train["Sex"] == 'female'].value_counts(normalize = True)[1]*100


print "------------------\n\n"
print train["Survived"][train["Sex"] == 'female']#We will get survived column's data only for females.
print "*****************\n\n"
print train["Survived"][train["Sex"] == 'female'].value_counts()

print "====================================\n\n"
print train["Survived"][train["Sex"] == 'female'].value_counts(normalize = True)#Data will change as a percentage in fraction of 1.
...
train['AgeGroup'] = pd.cut(train["Age"], bins, labels = labels)#There will be 13th column i.e. age group.
...
train["CabinBool"] = (train["Cabin"].notnull().astype('int'))#14th column is introduces which tells whether cabin no. is known or not.
...
train = train.drop(['Cabin'], axis = 1)#Data cleaning. To delete column of cabin no.
test = test.drop(['Cabin'], axis = 1)

#Ticket Feature
#we can also drop the Ticket feature since it's unlikely to yield any useful information
train = train.drop(['Ticket'], axis = 1)# to delete ticket no. column.
...
for dataset in combine:
    dataset['Title'] = dataset['Name'].str.extract(' ([A-Za-z]+)\.', expand=False)#Title starts from <space> and ends at '.'.
                                                            #+ means 1 or more repetation. '\' means treat '.' as a character instead of any one.
#Title is the 15th column.
print "\n\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n"
print train
print
...
print pd.crosstab(train['Title'], train['Sex'])#To find male/female considering their subnames.
...
for x in range(len(test["Fare"])):
    if pd.isnull(test["Fare"][x]):
        ...
        test["Fare"][x] = round(train[train["Pclass"] == pclass]["Fare"].mean(), 4)#Round off to two digits after decimal point.

#map Fare values into groups of numerical values
train['FareBand'] = pd.qcut(train['Fare'], 4, labels = [1, 2, 3, 4])#To arrange fare in sorted order and dividing it in 4 categories.
